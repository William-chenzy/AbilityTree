...
bool CloudViewer::MatToCloudPoint(cv::Mat& img, std::string name, int base_z, int num_z, int space_z) {
	...
	for (int i = 0; i < img.rows; i++) {
		for (int j = 0; j < img.cols; j++) {
			...
			if (val[3] == 0)continue;
			float x = i - half_row, y = j - half_col, z = base_z;
			float r = val[0] / 255.f, g = val[1] / 255.f, b = val[2] / 255.f;
			ColorPoint _dat{ x, y, z, r, g, b };
...
void CloudViewer::RemoveBaseValue(std::string name) {
	gl_ptr->RemoveBaseValue(name);
	UpdataOnce();
}

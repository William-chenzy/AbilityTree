...
string GetEtProtocol(ushort et) {
	ushort et_t = (ushort)((et & 0x00FFU) << 8 | (et & 0xFF00U) >> 8);
	switch (et_t) {
	...
	case 0x8864: return "PPPoE";          // PPPoE（以太网上的点对点协议）
...
string GetIntProtocol(uchar pt) {
	...
	case 59:  return "No Next Header"; // IPv6 no next header
	case 60:  return "Destination Options"; // IPv6 Destination Options
	case 41:  return "IPv6";           // IPv6 in IPv4 (IP protocol number)
	default:  return to_string((int)pt);
	}
}
...
void Wireshark::AnalysisPackageHead(std::list<PACKAGE>::iterator package) {
	...
	package->head.eth.protocol = GetEtProtocol(p_data[12]).c_str();
	...
	string net_pro = GetIntProtocol(p_data[23]) + "(" + to_string((int)p_data[23]) + ")";
